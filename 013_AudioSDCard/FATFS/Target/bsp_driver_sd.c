...
__weak uint8_t BSP_SD_IsDetected(void)
{
  ...
  __HAL_RCC_GPIOC_CLK_ENABLE();  // Ganti dengan port CD pin Anda jika berbeda
  GPIO_InitStruct.Pin = GPIO_PIN_13;  // Contoh: PC13 sebagai CD pin (sesuaikan dengan board Anda, misalnya Nucleo-144 punya PC13 untuk SD detect)
  GPIO_InitStruct.Mode = GPIO_MODE_INPUT;
  GPIO_InitStruct.Pull = GPIO_PULLUP;  // Pull-up jika CD pin active low
  HAL_GPIO_Init(GPIOC, &GPIO_InitStruct);

  if (HAL_GPIO_ReadPin(GPIOC, GPIO_PIN_13) == GPIO_PIN_RESET)  // Active low: RESET berarti SD present
  {
    return SD_PRESENT;
  }
